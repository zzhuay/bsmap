...
parser.add_option("-l", "--light", dest="light_mode", type="int", help="0: all information. 1: only chr, pos, total_C, methy_C. 2: 1+CpG only. [default: 1]", default=1)
...
if options.light_mode == 0:
    ...
    fout = open(options.outfile, 'w')
    # z95, z95sq = 1.96, 1.96 * 1.96
    fout.write('chr\tpos\ttotal_C\tmethy_C\n')
    nc, nd, dep0 = 0, 0, options.min_depth
    
    if options.light_mode != 2:
        for cr in sorted(depth.keys()):
            depthcr, methcr, refcr = depth[cr], meth[cr], ref[cr]
            for i, d in enumerate(depthcr):
                if d < dep0: continue
        ...
        for cr in sorted(depth.keys()):
            ...
            for i, d in enumerate(depthcr):
                ...
                # ratio = float(m) / d
                seq = refcr[i-1:i+2]
                if seq[:2]!='CG' and seq[1:]!='CG': continue
                # strand = ss[refcr[i]]
                # pmid = ratio + z95sq / (2 * d)
                # sd = z95 * ((ratio*(1-ratio)/d + z95sq/(4*d*d)) ** 0.5)
                # norminator = 1 + z95sq / d
